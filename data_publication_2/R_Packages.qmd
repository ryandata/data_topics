...
Now we run _load_all_ to bring our functions as we've defined them into the R workspace.  Note that we remove the previous manually created functions just to show that _load_all_ is working properly.  
We also reconfirm that we're in the correct project directory.  Then we can check that the functions are in our workspace, now provided directly by the package defintions.
```{r load_all}
rm(funkyadd)
rm(randomadd)
